...
from ZS_KGC.models.DOZSL_GAN.args import read_options
from ZS_KGC.models.DOZSL_GAN.data_loader import Extractor_generate, centroid_generate, train_generate_decription
from ZS_KGC.models.DOZSL_GAN.Networks import Extractor, Generator, Discriminator
...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        rela_matrix = np.load(os.path.join(self.data_path, 'embeddings', self.semantic_of_rel))['relaM']
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE']:
            ...
            np.savez(os.path.join(self.data_path, self.trained_embed_path, self.embed_model), embeddings)
            json.dump(symbol_id, open(os.path.join(self.data_path, self.trained_embed_path, self.embed_model + '2id'), 'w'))
    ...
    def read_embed(self):
        ...
        symbol_id = json.load(open(
                os.path.join(self.data_path, self.trained_embed_path, self.embed_model + '2id')))
        embeddings = np.load(os.path.join(self.data_path, self.trained_embed_path,
                                              self.embed_model + '.npz'))['arr_0']
    ...
    def save_pretrain(self):
        torch.save(self.Extractor.state_dict(), self.save_path + '/' + self.embed_model + '_Extractor')


    def load_pretrain(self):
        self.Extractor.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_Extractor', map_location=lambda storage, loc: storage.cuda(self.args.device)))
        self.Extractor.eval()
    ...
    def save_model(self):
        ...
        torch.save(self.Generator.state_dict(), path + '/' + self.embed_model + '_Generator')
        torch.save(self.Discriminator.state_dict(), path + '/' + self.embed_model + '_Discriminator')

    def load_model(self):
        self.Generator.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_Generator'))
        self.Discriminator.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_Discriminator'))
    ...
    def train(self):
        ...
        for epoch in range(1, (self.train_times+1)):
            ...
            if epoch >= self.start_eval and epoch % self.eval_every == 0:
    ...
    def eval(self, mode='dev', meta=True, epoch=0):
        ...
        # test_candidates = json.load(open(self.data_path + "/test_candidates_sub_10.json"))
