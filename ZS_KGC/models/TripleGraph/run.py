...
class Runner:
    def __init__(self, args):
        ...
        # prepare graph
        self.sp_nodes_dict, self.sp_adj, self.sp_inputs = build_graph(args, meta_rel='rdfs:subPropertyOf')
        self.domain_nodes_dict, self.domain_adj, self.domain_inputs = build_graph(args, meta_rel='rdfs:domain')
        self.range_nodes_dict, self.range_adj, self.range_inputs = build_graph(args, meta_rel='rdfs:range')


        print("Loading Training Data ... ")
        fc_vectors = self.prepare_train_data()

        self.sp_train_idx, self.domain_train_idx, self.range_train_idx \
            = [self.sp_nodes_dict[query] for query in self.seen_rels], \
              [self.domain_nodes_dict[query] for query in self.seen_rels], \
              [self.range_nodes_dict[query] for query in self.seen_rels]

        self.sp_test_idx, self.domain_test_idx, self.range_test_idx \
            = [self.sp_nodes_dict[query] for query in self.unseen_rels], \
              [self.domain_nodes_dict[query] for query in self.unseen_rels], \
              [self.range_nodes_dict[query] for query in self.unseen_rels]
        ...
        self.sp_inputs = F.normalize(torch.tensor(self.sp_inputs).float().cuda())
        self.domain_inputs = F.normalize(torch.tensor(self.domain_inputs).float().cuda())
        self.range_inputs = F.normalize(torch.tensor(self.range_inputs).float().cuda())


        self.sp_adj = self.sp_adj.cuda()
        self.domain_adj = self.domain_adj.cuda()
        self.range_adj = self.range_adj.cuda()
    ...
    def train(self, epoch):
        ...
        sp_output_idx = self.sp_train_idx + self.sp_test_idx
        domain_output_idx = self.domain_train_idx + self.domain_test_idx
        range_output_idx = self.range_train_idx + self.range_test_idx
        n_train = len(self.sp_train_idx)

        alpha = 1/3
        # Start training
        self.model.train()
        sp_emb_train = self.model(self.sp_inputs, self.sp_adj)[self.sp_train_idx]
        domain_emb_train = self.model(self.domain_inputs, self.domain_adj)[self.domain_train_idx]
        range_emb_train = self.model(self.range_inputs, self.range_adj)[self.range_train_idx]

        embed_train = alpha * sp_emb_train + alpha * domain_emb_train + alpha * range_emb_train


        loss = self.l2_loss(embed_train, self.labels)
        ...
        if epoch % args.evaluate_epoch == 0:
            with torch.no_grad():
                ...
                sp_emb_test = self.model(self.sp_inputs, self.sp_adj)[sp_output_idx]
                domain_emb_test = self.model(self.domain_inputs, self.domain_adj)[domain_output_idx]
                range_emb_test = self.model(self.range_inputs, self.range_adj)[range_output_idx]

                embed_test = alpha * sp_emb_test + alpha * domain_emb_test + alpha * range_emb_test

                loss_test = self.l2_loss(
                    embed_test[:n_train], self.labels
                )
                print('epoch {}, train_loss={:.4f}'.format(epoch, loss_test.item()))
                if epoch % 50 == 0 and epoch >= args.test_epoch:
                    output_vectors = embed_test[n_train:]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_epoch', type=int, default=800)
