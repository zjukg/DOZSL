...
class DOZSL_AGG(CapsuleBase):
    ...
    def forward(self, sub, rel, mode='train'):
        ...
        # calculate the score
        sub_emb_selected = torch.zeros([sub_emb.size(0), sub_emb.size(-1)], dtype=torch.float).to(self.device)
        ...
        x_selected = torch.zeros([x.size(0), x.size(-1)], dtype=torch.float).to(self.device)
