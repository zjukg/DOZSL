...
class DisenE(CapsuleBase):
    def __init__(self, params=None, device=None):
        ...
        self.fc1 = torch.nn.Linear(self.p.embed_dim * 3, 1)
        self.non_linearity = torch.nn.ReLU()
        self.softmax = torch.nn.Softmax(dim=-1)
        nn.init.xavier_uniform_(self.fc1.weight, gain=1.414)

        self.fc_att = get_param((self.p.num_rel, self.p.num_factors))
    ...
    def forward(self, sub, rel, mode='train'):
        ...
        rel_emb = rel_emb.view(-1, self.p.num_factors, self.p.embed_dim)  # [B,K,F]
