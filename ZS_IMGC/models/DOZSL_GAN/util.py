...
class DATA_LOADER(object):
    ...
    def read_imagenet(self, args):
        ...
        seen_index = self.ID2Index(wnids, os.path.join(args.DATADIR, 'ImageNet', args.DATASET, 'seen.txt'))
        unseen_index = self.ID2Index(wnids, os.path.join(args.DATADIR, 'ImageNet', args.DATASET, 'unseen.txt'))
        ...
        matcontent = scio.loadmat(os.path.join(args.DATADIR, 'ImageNet', args.DATASET, args.SemFile))
        embed = matcontent['embeddings']
        self.semantic = torch.from_numpy(embed).float()

        print("semantic embedding shape:", self.semantic.shape)
    ...
    def read_dataset(self, args):
        ...
        # load split.mat
        att_matcontent = scio.loadmat(os.path.join(args.DATADIR, args.DATASET, args.SplitFile))
        # numpy array index starts from 0, matlab starts from 1
        trainval_loc = att_matcontent['trainval_loc'].squeeze() - 1
        # train_loc = matcontent['train_loc'].squeeze() - 1
        # val_unseen_loc = matcontent['val_loc'].squeeze() - 1
        test_seen_loc = att_matcontent['test_seen_loc'].squeeze() - 1
        test_unseen_loc = att_matcontent['test_unseen_loc'].squeeze() - 1
