...
class Runner:
    def __init__(self, args):
        ...
        self.hie_adj, self.att_adj, self.y = self.data.hie_adj, self.data.att_adj, self.data.fc_vectors
        self.hie_inputs, self.att_inputs = self.data.hie_inputs, self.data.att_inputs

        self.hie_train_idx, self.hie_test_idx = self.data.hie_train_idx, self.data.hie_test_idx
        self.att_train_idx, self.att_test_idx = self.data.att_train_idx, self.data.att_test_idx
        ...
        self.hie_inputs = torch.tensor(self.hie_inputs).float().cuda()
        self.hie_inputs = F.normalize(self.hie_inputs)

        self.att_inputs = torch.tensor(self.att_inputs).float().cuda()
        self.att_inputs = F.normalize(self.att_inputs)


        self.hie_adj = self.hie_adj.cuda()
        self.att_adj = self.att_adj.cuda()
    ...
    def train(self, epoch):
        ...
        hie_output_idx = self.hie_train_idx + self.hie_test_idx
        att_output_idx = self.att_train_idx + self.att_test_idx

        n_train = len(self.hie_train_idx)

        self.model.train()
        hie_output_vectors = self.model(self.hie_inputs, self.hie_adj)[self.hie_train_idx]
        att_output_vectors = self.model(self.att_inputs, self.att_adj)[self.att_train_idx]

        output_vectors = args.alpha * hie_output_vectors + (1 - args.alpha) * att_output_vectors

        # calculate the loss over training seen nodes
        loss = self.l2_loss(output_vectors, self.labels)
        ...
        if epoch % args.evaluate_epoch == 0:
            ...
            hie_output_vectors = self.model(self.hie_inputs, self.hie_adj)[hie_output_idx]
            att_output_vectors = self.model(self.att_inputs, self.att_adj)[att_output_idx]
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--DATASET', default='ImageNet/ImNet_A', help='the folder to store subset files')
    ...
    # parameter for Dual Graph
    parser.add_argument('--alpha', default=0.5, type=float, help='weight for combing two graphs')
